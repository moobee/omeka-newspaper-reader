...
class SolrSearch_Helpers_Index
{
    ...
    /**
     * Effectue une requête sur Solr
     */
    private static function requestViaHandler($handler, $queryString) {
        $url = 'http://' . SOLR_HOST . ':8080/solr/omeka/' . $handler . $queryString;
        ...
        curl_setopt($ch, CURLOPT_USERPWD, SOLR_LOGIN . ':' . SOLR_PASS);
    ...
    /**
     * Indexe tous les documents via l'import handler de Solr
     */
    public static function indexAllViaHandler() {
        self::requestViaHandler('dataimport', '?entity=document&commit=true&command=full-import&clean=true');
    }
    ...
    public static function getStatusViaHandler() {
        $status = self::requestViaHandler('dataimport', '?command=status&wt=json');
        return $status;
    }
